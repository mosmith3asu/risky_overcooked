...
class CustomQAgent(Agent):
    ...
    def action(self, state,rationality=10.0):
...
if __name__ == "__main__":
    ...
    for iter in range(ITERATIONS):
        ...
        if len(iter_rewards) % 10 == 0:
            ...
            print(f"Iteration {iter} complete | 10-Mean {np.mean(iter_rewards[-10:])} | Qminmax {q_agent.Q_table.min()} {q_agent.Q_table.max()} | P(explore) {q_agent.exploration_proba}")
