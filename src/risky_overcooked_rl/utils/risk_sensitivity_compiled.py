...
class CumulativeProspectTheory:
    ...
    Example Lecture: https://engineering.purdue.edu/DELP/education/decision_making_slides/Module_12___Cumulative_Prospect_Theory.pdf
    """
    ...
    def expectation_samples(self, all_next_q_value, all_p_next_states, prospect_mask, reward, gamma):
        ...
        if self.is_rational:
            return rat_expactation_samples_jit(expected_td_targets,
...
def rat_expactation_samples_jit(expected_td_targets,prospect_next_values, prospect_p_next_states,prospect_masks,reward,gamma):
    ...
    for i in range(BATCH_SIZE):
        ...
        expected_td_targets[i] = np.sum(prospect_td_targets.flatten()*prospect_probs.flatten())
...
def expactation_samples_jit(expected_td_targets,prospect_next_values, prospect_p_next_states,prospect_masks,reward,gamma,
    ...
    def expectation(values, p_values, b, lam, eta_p, eta_n, delta_p, delta_n):
        ...
        def rho_neg(sorted_v, Fk, l, K, b, lam, eta_n, delta_n):
            _rho_n = 0
        ...
        def rho_plus(sorted_v, Fk, l, K, b, eta_p, delta_p):
            _rho_p = 0
        ...
        rho_p = rho_plus(sorted_v, Fk, l, K,b, eta_p, delta_p)
        rho_n = rho_neg(sorted_v, Fk, l, K, b, lam, eta_n, delta_n)
    ...
    for i in range(BATCH_SIZE):
        ...
        if mean_val_ref: b = np.mean(prospect_td_targets)
        expected_td_targets[i] = expectation(prospect_td_targets.flatten(), prospect_probs.flatten(),
                                             b, lam, eta_p, eta_n, delta_p, delta_n)
        if mean_val_ref: expected_td_targets[i] += b
    return expected_td_targets
# @jit(fastmath=True)
@jit
