...
config = {
        ...
        'LAYOUT': "sanity_check3_single", 'HORIZON': 800, 'ITERATIONS': 2_000,
        ...
        # "learning_rate": 1e-3,
        "learning_rate": 1e-4,
        'rationality': 'max',#10, #  base rationality if not specified (i.e. testing), ='max' for argmax
...
if __name__ == "__main__":
    ...
    # Generate agents
    q_agent = SoloDeepQAgent(mdp,agent_index=0,model=DQN,config=config)
    ...
    for iter in range(ITERATIONS):
        ...
        exp_rationality = 'random' if np.random.uniform(0, 1)  < exploration_proba else 'max'
        ...
        if iter % test_interval == 0:
            ...
            for test in range(N_tests):
                ...
                for t in count():
                    ...
                    state = env.state
                    action1, _ = q_agent.action(state,rationality=config['rationality'])
