import numpy as np
from risky_overcooked_rl.algorithms.DDQN.utils.agents import device,ResponseAgent
...
#
#     def init_sched(self,config,eps_decay = 1,rshape_decay=1,
#                    eps_sched=None,rshape_sched=None,rational_sched=None,rand_start_sched=None):
#         EPS_START, EPS_END, EPS_DUR = config['epsilon_sched'] if eps_sched is None else eps_sched
#         RAT_START, RAT_END, RAT_DUR = config['rationality_sched'] if rational_sched is None else rational_sched
#         RSHAPE_START, RSHAPE_END, RSHAPE_DUR = config['rshape_sched'] if rshape_sched is None else rshape_sched
#         RSTART_START, RSTART_END, RSTART_DUR = config['rand_start_sched'] if rand_start_sched is None else rand_start_sched
#
#         EPS_START = EPS_START * eps_decay
#         RSHAPE_START = RSHAPE_START * rshape_decay
#
#         self.test_rationality = RAT_END
...
class ResponseTrainer():
    def __init__(self,partner_model_object,config):
        ...
        config['obs_shape'] = None  # defined later
        config['device'] = device
        config['AGENT'] = ResponseAgent.__name__
        config['Date'] = datetime.now().strftime("%m_%d_%Y-%H_%M")
        self.config = config
        self.cpt_params = config['cpt_params']
        self.timestamp = config['Date']
    ...
    def run(self):
        ...
        for it in range(self.ITERATIONS):
            ...
            if it > 1: self.ego_agent.scheduler.step()  # updates learning rate scheduler
            self.ego_agent.update_target()  # performs soft update of target network
            self.logger.end_iteration()
            ...
            print(f"Iteration {it} "
                  ...
                  f"| handoffs:{handoffs} "
                  f" |"
                  f"| mem:{len(self.ego_agent._memory)} "
                  f"| rshape:{round(self.rshape_sched[it], 3)} "
                  f"| rat:{round(self.rationality_sched[it], 3)}"
                  f"| eps:{round(self.epsilon_sched[it], 3)} "
                  f"| LR={round(self.ego_agent.optimizer.param_groups[0]['lr'], 4)}"
                  f"| rstart={round(self.random_start_sched[it], 3)}"
                  )
    ...
    def training_rollout(self, it, rationality, epsilon, rshape_scale, p_rand_start=0):
        ego,partner = 0,1
        self.ego_agent.rationality = rationality
        self.env.reset()
        ...
        for t in count():
            ...
            feasible_JAs = self.feasible_action.get_feasible_joint_actions(self.env.state, as_joint_idx=True)
            joint_action, joint_action_idx, action_probs = self.ego_agent.choose_joint_action(obs, epsilon=epsilon, feasible_JAs=feasible_JAs)
            next_state, reward, done, info = self.env.step(joint_action, get_mdp_info=True)
            ...
            # Store in memory ----------------
            # self.ego_agent.memory_double_push(state=obs,
            #                               action=joint_action_idx,
            #                               rewards=total_rewards,
            #                               next_prospects=next_state_prospects,
            #                               done=done)
            self.ego_agent._memory.push(state=obs,
                                          action=joint_action_idx,
                                          reward=total_rewards[ego],
                                          next_state=next_obs,
                                          done=done)
            # Update model ----------------
            loss = self.ego_agent.update()
            if loss is not None: losses.append(loss)
            if done: break
            self.env.state = next_state
            obs = next_obs
    ...
    def test_rollout(self, rationality, epsilon=0, rshape_scale=1, get_info=False):
        ...
        self.ego_agent.model.eval()
        self.ego_agent.target.eval()
        self.ego_agent.rationality = rationality
        self.env.reset()
        ...
        for t in count():
            obs = self.mdp.get_lossless_encoding_vector_astensor(self.env.state, device=device).unsqueeze(0)
            joint_action, joint_action_idx, action_probs = self.ego_agent.choose_joint_action(obs, epsilon=epsilon)
            next_state, reward, done, info = self.env.step(joint_action)
            ...
            if done:  break
            self.env.state = next_state
        self.ego_agent.model.train()
        self.ego_agent.target.train()
        if get_info: return test_reward, test_shaped_reward, state_history, action_history, aprob_history, rollout_info
        else:  return test_reward, test_shaped_reward, state_history, action_history, aprob_history
    def checkpoint(self,it, state_history):
        ...
        if score > self.checkpoint_score:
            print(f'\nCheckpointing model at iteration {it} with score {score}...\n')
            self.ego_agent.update_checkpoint()
            self.logger.update_checkpiont_line(it)
            self.checkpoint_score = score
            self.has_checkpointed = True
