...
class SoloDeepQAgent(Agent):
    ...
    def __init__(self, mdp, agent_index,
        ...
        self.invert_feature_vector = (self.my_index==1) # reverse feature order if player 2

        # Learning Params ---------------
        self.gamma = config['gamma']
        self.lr = config['learning_rate']
        self.tau = config['tau'] # target network soft update rate
        ...
        # self.model = DQN_vector_feature(**config)  # DQN model used for learning
    ...
    def action(self, obs,rationality=None,epsilon=None):
        """ Dynamic action selection with either epsilon-greedy or boltzmann exploration
        obs: featurized OvercookedState (e.g., np vector, not the object)
        """
        assert rationality is not None or epsilon is not None, "Must specify rationality or epsilon"

        # Check if player 2 -- invert player feature locations
        # if self.invert_feature_vector:
        #     obs = np.flip(obs)


        # If using eps-greedy then set
        if epsilon is not None:
            sample = random.random()
            rationality = 'random' if sample < epsilon else 'max'

        # Greedy Agent -----------
        if rationality == 'max':
            with torch.no_grad():
                # obs = self.featurize(state)
                # check if obs is a tensor
        ...
        # Boltzmann Sampling -----
            ...
            with torch.no_grad():
                # obs = self.featurize(state)
                obs = torch.tensor(obs, dtype=torch.float32, device=self.device).unsqueeze(0)
                qA = self.policy_net(obs).numpy().flatten()[0]
    ...
    def update(self, experiences, invert_player=False):
    ...
    def featurize(self, overcooked_state):
        ...
        if self.featurize_fn.lower() in ['mask', 'lossless']:
            ...
            return self.mdp.get_lossless_vector_encoding(overcooked_state,n_players=self.n_players)
...
#     ########################################################
#
#     def action(self, obs,enable_explore=True,rationality=None,epsilon=None):
#         """ Dynamic action selection with either epsilon-greedy or boltzmann exploration
#         obs: featurized OvercookedState (e.g., np vector, not the object)
#         """
#         assert rationality is not None or epsilon is not None, "Must specify rationality or epsilon"
...
# DEPRECATED ########################################################
    ...
    #             self.load_config(config, verbose=verbose_load_config)
    #         else:
    #             warnings.warn("No config provided, using default values. Please ensure defaults are correct")
    #
    #         # self.model = model(**config)  # DQN model used for learning
    #         self.model = DQN(**config)  # DQN model used for learning
    #
    #     def load_config(self, config, verbose=True):
    #         if verbose:
    #             print(f'--------------------------------------------------')
    ...
    #
    #     def print_config(self):
    #         report_list = ['exploration_proba', 'max_exploration_proba', 'min_exploration_proba', 'gamma', 'lr',
    #                        'rationality']
    #         print(f'--------------------------------------------------')
    #         print(f'Agent {self.my_index} Config:')
    #         print(f'--------------------------------------------------')
    ...
    #         player = state.players[player_idx]
    #         features[f'player_{player_idx}_loc'][player.position] = 1
    #         features[f'player_{player_idx}_orientation_{Direction.DIRECTION_TO_INDEX[player.orientation]}'][
    #             player.position] = 1
    #
    #         # Joint player observation ----------------
    #         # for player_idx, player in enumerate(state.players):
    ...
    #
    #             elif obj.name == "dish":
    #                 features["dishes"][obj.position] += 1
    #             elif obj.name == "onion":
    #                 features["onions"][obj.position] += 1
    #             # elif obj.name == "tomato": features["tomatoes"][obj.position] += 1
    #             else:
